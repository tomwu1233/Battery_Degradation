...
class nmc_20Ah(BatteryDegradationModel):
    """
    半经验电池退化模型实现，基于 Nájera et al. (2023) Table 4 中 SANYO UR18650 W 参数。
    同时考虑日历衰减和循环衰减。
    """
    def __init__(self, degradation_scalar: float = 1, label: str = "NMC 20Ah"):  
        # 内部状态
        self.states = {
    ...
    @property
    def cap(self):
        return 20  # 20 Ah 大容量电池

    @property
    def _params_life(self):
        # 来自论文 Table 4 for NMC 20 Ah - 调整后参数
        # 注意：原始参数针对 3Ah 18650 电池，现调整为 20Ah 大容量电池
        # 单位系统确认：
        # 温度 T：开尔文 (K)
        # 时间 t：天 (days) 
        # AH（安时）：Ah
        ...
        # 模型输出 Q_loss：百分比 (%)
        return {
            'a': 1.2787e-6,    # 循环衰减系数 (增加10倍)
            'b': -7.6845e-4,   # 循环衰减系数 (增加10倍)
            'c': 0.116,        # 循环衰减系数 (增加10倍)
            'd': -0.0065,      # C‑rate 温度系数 (保持不变)
            'e': -1.3745,      # C‑rate 常数系数 (保持不变)
            'f': 1.1363e13,    # 日历衰减基础系数 (增加10倍)
            'g': 4.6996,       # SoC 指数系数 (保持不变)
            'h': -1.0772e4,    # 温度指数系数 (保持不变)
            'z': 0.5,          # 时间幂指数（保持不变）
        }
    ...
    def update_rates(self, stressors):
        ...
        if 'I' in stressors and not np.isnan(stressors['I']).all():
            ...
            # 从 Crate 估算电流，考虑当前电池容量衰减
            # 改进：智能默认值策略
            if 'Crate' in stressors and not np.isnan(stressors['Crate']):
                Crate = stressors['Crate']
            else:
                # 智能默认值：根据 SOC 变化判断状态
                if 'soc' in stressors and len(stressors['soc']) > 1:
                    soc_change = np.max(stressors['soc']) - np.min(stressors['soc'])
                    if soc_change < 0.01:  # SOC 变化小于 1%
                        Crate = 0.0  # 静置状态
                    else:
                        Crate = 0.5  # 正常使用的典型C率
                else:
            ...
            # 使用当前实际容量（考虑衰减）
            current_capacity = self.cap * self.outputs['q'][-1]  # Ah
            
            if isinstance(Crate, (int, float)):
                I = np.full_like(soc, Crate * current_capacity)  # 基于实际容量计算电流
            else:
                I = Crate * current_capacity
        
        # 确保 Crate 存在且有效
        if 'Crate' in stressors and not np.isnan(stressors['Crate']):
            Crate = stressors['Crate']
        else:
            # 从电流估算 Crate，使用当前实际容量
            current_capacity = self.cap * self.outputs['q'][-1]  # Ah
            if hasattr(I, '__len__'):
                Crate = np.mean(np.abs(I)) / current_capacity
            else:
                Crate = np.abs(I) / current_capacity
        ...
        if 'delta_ah' in stressors and not np.isnan(stressors['delta_ah']):
            ...
            # 从 delta_efc 估算 delta_ah，使用当前实际容量
            delta_efc = stressors.get('delta_efc', 0.0)
            if np.isnan(delta_efc):
                delta_efc = 0.0
            current_capacity = self.cap * self.outputs['q'][-1]  # 考虑衰减后的容量 (Ah)
            delta_ah = delta_efc * current_capacity  # 转换为 Ah
