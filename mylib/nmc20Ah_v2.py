...
class nmc_20Ah(BatteryDegradationModel):
    ...
    def _params_life(self):
        # 基于论文 Table 4，调整适应 20Ah 大容量电池
        # 注意：原始参数针对 3Ah 18650 电池，现调整退化参数适应大容量
        # 单位系统确认：
        # 温度 T：开尔文 (K)
        # 时间 t：天 (days) 
        # AH（安时）：Ah
        ...
        # 模型输出 Q_loss：百分比 (%)
        return {
            'a': 1.2787e-6,    # 循环衰减系数 (增加10倍，适应大容量)
            'b': -7.6845e-4,   # 循环衰减系数 (增加10倍，适应大容量)
            'c': 0.116,        # 循环衰减系数 (增加10倍，适应大容量)
            'd': -0.0065,      # C‑rate 温度系数 (保持不变)
            'e': -1.3745,      # C‑rate 常数系数 (保持不变)
            'f': 1.1363e13,    # 日历衰减基础系数 (增加10倍，适应大容量)
    ...
    def update_rates(self, stressors):
        ...
        if 'I' in stressors and not np.isnan(stressors['I']).all():
            ...
            # 保持原有 Crate 处理逻辑不变
            Crate = stressors.get('Crate')
            
            # 使用当前实际容量（考虑衰减）- 单位统一为 Ah
