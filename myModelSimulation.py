...
#import my fit model
# nmc_SANYOUR18650 and nmc_20ah are from literature
# nmc111_samsung94ah is the one I fitted into semi_empirical model
# nmc111_samsung94ah_SR is the one I fitted into Symbolic Regression model

from mylib import nmc_SANYOUR18650, nmc_20Ah, nmc111_samsung94ah, nmc111_samsung94ah_SR
from mylib.nmc111_samsung94ah import NMC111_SAMSUNG94AH
from mylib.nmc111_samsung94ah_SR import NMC111_SAMSUNG94AH_SR
...
pv_15min_series = pv_hourly_kW.resample("15min").ffill().reindex(base_index, method='ffill') * 0.25 # Convert power (kW) to energy (kWh)
logger.info(f"Fetched PV generation data. Annual total: {pv_15min_series.sum():.2f} kWh")
...
cell = NMC111_SAMSUNG94AH_SR()   # Here to change the required cell, crucial part!
...
for q in range(total_quarters):
    ...
    T_celsius = np.full_like(slice_soc, 60) #25
...
# == Stressors
stress_df = pd.DataFrame(stress_history)
stress_df['year'] = np.arange(len(stress_df)) / 4  # 每季度为0.25年
print("\n--- 压力参数统计 ---")
print(stress_df.describe())
plt.figure(figsize=(10, 6))
plt.plot(stress_df['year'], stress_df['soc'], label='Average SOC')
plt.plot(stress_df['year'], stress_df['dod'], label='Average DOD')
plt.plot(stress_df['year'], stress_df['Crate'], label='Crate (95th percentile)')
plt.xlabel('Year')
plt.ylabel('Value')
plt.title('Quarterly Battery Stress Parameters Over 15 Years')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
